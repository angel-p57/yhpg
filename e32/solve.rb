def solve input
  ...
  (1..35).each{|y|
    x=1
    loop {
      x+=1 while x<36&&(field[y][x]==field[y][x-1]||field[y][x]==field[y-1][x])
      break if x>=36
      curh=field[y][x]
      xe,ye=x+1,y+1
      xe+=1 while xe<36&&field[y][xe]==curh
      ye+=1 while ye<36&&field[ye][x]==curh
      sqs.push((xe-x)*(ye-y)) \
        if (y+1...ye).all?{|yy| (x+1...xe).all? {|xx| field[yy][xx]==curh } } &&
           (x+1...xe).all?{|xx| field[y-1][xx]!=curh&&field[ye][xx]!=curh } &&
           (y+1...ye).all?{|yy| field[yy][x-1]!=curh&&field[yy][xe]!=curh }
      x=xe
    }
  }
  sqs.sort*?,
