def solve input
  ...
  cand=(0...b).map{|e| e.to_s(b) }
  ms,ns=[m,n].map{|e| e.to_s(b) }
  (f=->l,u,y,d=0{
    if l==""
      return "" if y==1
      l="0"
    ...
    lt,ld,ut,ud=[l,u].flat_map{|e| [e[0],e.size] }
    #p [l,u,y,d,lt,ld,ut,ud]
    cand.drop(d).each{|i|
      ts=(b**(ud-1)-b**(ld-1))/(b-1)
      if i<lt
        ts-=b**(ld-1)
      elsif i==lt
        ts-=l.to_i(b)-i.to_i(b)*b**(ld-1)
        #p ["**",ts]
      end
      if i<ut
        ts+=b**(ud-1)
      elsif i==ut
        ts+=u.to_i(b)-i.to_i(b)*b**(ud-1)+1
      end
      #p [i,ts]
      next if ts<0
      next y-=ts if y>ts
      ln=i<lt ? "0"*ld : i==lt ? l[1..-1] : "0"*(ld-1)
      un=i<ut ? cand[-1]*(ud-1) : i==ut ? u[1..-1] : cand[-1]*(ud-2)
      return i+f[ln,un,y]
    }
    raise "not reached"
  })[ms,ns,x,1].to_i(b)
